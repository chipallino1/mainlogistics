function getSearchResults() {
    ...
    if(defaultInline1.checked){
        getSearchFirms("/search/firms?firmName=");
    }
}
...
function getSearchRoutePage(e) {
    ...
    if(e.target.id=='lastPage'+state){
        ...
        if(window.location.pathname=="me")
            getUserRoutes("/search/routes?page="+last+"&countryFrom="+countryFrom.value+"&cityFrom="+cityFrom.value+"&countryTo="+countryTo.value+"&cityTo="+cityTo.value,getSearchRoutesResult);
        else
            getUserRoutes("/search/routes?page="+last+"&countryFrom="+countryFrom.value+"&cityFrom="+cityFrom.value+"&countryTo="+countryTo.value+"&cityTo="+cityTo.value,getSearchRoutesResult);
        return;
    }
